...
seoDescription: "My setup for managing different Node.js versions and package managers per project using fnm and Corepack."
datePublished: 'Wed Jan 17 2024 19:23:41 GMT+0000 (Coordinated Universal Time)'
slug: nodejs-and-package-managers-versions
dateLastModified: '2026-01-05'
...
<Callout>
**Update (January 2026):** Corepack was removed from Node.js 25+ (TSC voted March 2025). It's still maintained but must now be installed separately via `npm install -g corepack`. The `FNM_COREPACK_ENABLED` flag is therefore less useful. fnm itself is now listed as an [official installation option on nodejs.org](https://nodejs.org/en/download).
</Callout>
...
The system-level Node.js version is also listed in `nvm ls` as `system`, but that is not my default version, so it never gets used for commands I run in the terminal.
