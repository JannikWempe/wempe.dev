...
I scratched my head and did what a developer does: ask Google.
...
Google told me it is related to pre-rendering and `ReactDOM`s hydration (also referred to as rehydration). I digged
...
**Static generation** _(the SG in SSG, which stands for Static Site Generator)_ and **Server-Side Rendering** (SSR).
...
Please note that this solution will make the `CartNavigationIcon` render twice. Do not overuse this for too many and / or
...
isn't too helpful to point the developer into the right direction. But maybe this article helps to not only resolve the
